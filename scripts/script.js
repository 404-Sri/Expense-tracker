...
//addtransactionDom
 const addTransactionDOM = function(transaction) {
     ...
     item.classList.add(transaction.amount < 0 ? 'minus' : 'plus');
    item.innerHTML = `${transaction.transaction}<span>${sign}${Math.abs(transaction.amount)}</span><button class="delete-btn" onClick = "removeTransaction(${transaction.id})">x</button>`;
 }
...
form.addEventListener('submit', (e) => {
    e.preventDefault();
    const transactions = {
        id: 1, transaction: transaction.value, amount: +amount.value
    };
